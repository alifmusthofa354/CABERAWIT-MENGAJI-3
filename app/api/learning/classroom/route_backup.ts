...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("classroom")
      .select("id, name, email, keterangan") // Menggunakan alias user:name untuk mengambil nama dari tabel user
      .eq("email", email);
...
const classSchema = z.object({
  ...
  description: z.string().max(255).optional(), // Description bersifat opsional dan maksimal 255 karakter
});
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("classroom")
      .insert([{ name, email, keterangan: description }]);
