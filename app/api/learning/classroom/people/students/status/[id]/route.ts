...
const AllowedStatusSchema = z.enum(["ACTIVE", "NON ACTIVE", "DELETE"], {
  message: "Status must be one of 'ACTIVE', 'NON ACTIVE', 'DELETE'",
});

// Konfigurasi Zod untuk data teks
const StudentUpdateSchema = z.object({
  status: AllowedStatusSchema,
  idStudentUpdate: z.string().uuid({ message: "Invalid ID User Classroom" }),
});

// Definisikan tipe literal untuk status yang diizinkan
type AllowedStatus = "DELETE" | "ACTIVE" | "NON ACTIVE";
...
export async function PATCH(
  ...
  try {
    ...
    const { status, idStudentUpdate } = validatedData;
    ...
    const allowedStatuses: AllowedStatus[] = ["ACTIVE", "NON ACTIVE", "DELETE"];
